...
/**
 * Implementation of a source which is described by an URL.
 *
...
public class URLSource extends AbstractSource implements Source {
    ...
    public void init(URL url, Map parameters) throws IOException {
        ...
        if (null != parameters) {
            m_parameters = (SourceParameters) parameters
                    .get(SourceResolver.URI_PARAMETERS);
            final String method = (String) parameters
                    .get(SourceResolver.METHOD);

            if ("POST".equalsIgnoreCase(method))
                m_isPost = true;

            final String encoding = (String) parameters
                    .get(SourceResolver.URI_ENCODING);
    ...
    /**
     * Get the last modification date and content length of the source. Any
     * exceptions are ignored. Override this to get the real information
     */
        ...
        if (!m_isPost) {
            try {
                if (null == m_connection) {
                    ...
                    try {
                        Method urlConnectionSetConnectTimeoutMethod = urlConnectionClass
                                .getMethod("setConnectTimeout",
                                        new Class[] { int.class });
                        Method urlConnectionSetReadTimeoutMethod = urlConnectionClass
                                .getMethod("setReadTimeout",
                                        new Class[] { int.class });
                        if (connectTimeout != -1)
                            urlConnectionSetConnectTimeoutMethod.invoke(
                                    m_connection, new Object[] { new Integer(
                                            connectTimeout) });
                        if (readTimeout != -1)
                            urlConnectionSetReadTimeoutMethod.invoke(
                                    m_connection, new Object[] { new Integer(
                                            readTimeout) });
                ...
                if (m_connection instanceof HttpURLConnection) {
                    ...
                    // IOException here!
                    try {
                        final int statusCode = ((HttpURLConnection) m_connection)
                                .getResponseCode();
     ...
     *             if file not found or HTTP location does not exist.
     * @throws IOException
    ...
    public InputStream getInputStream() throws IOException, SourceException {
        ...
        if (m_connection == null) {
            ...
            if (m_connection instanceof HttpURLConnection && m_isPost) {
                ...
                if (buffer.length() > 1) { // only post if we have parameters
                    ...
                    // request
                    httpCon.setRequestProperty("Content-length", Integer
                            .toString(postString.length()));
        ...
        m_connection = null; // make sure a new m_connection is created next
                                // time
        return input;
    }

    /**
     * Get the Validity object. This can either wrap the last modification date
     * or the expires information or... If it is currently not possible to
     * calculate such an information <code>null</code> is returned.
     */
